...
import type { YGCachedMeasurement } from './internal';
import {
  dim,
  kDefaultFlexGrow,
  kDefaultFlexShrink,
...
import type { YGStyle } from './ygstyle';
...
export function YGConfigFree(_config: YGConfig): void {
  gConfigInstanceCount--;
}
...
export function YGNodeStyleSetMinWidth(node: YGNode, minWidth: number): void {
  ...
  if (
    (node.getStyle().minDimensions[YGDimension.Width].value != value.value && value.unit != YGUnit.Undefined) ||
...
export function YGNodeStyleSetMinHeight(node: YGNode, minHeight: number): void {
  ...
  if (
    (node.getStyle().minDimensions[YGDimension.Height].value != value.value && value.unit != YGUnit.Undefined) ||
...
export function YGNodeStyleSetMaxWidth(node: YGNode, maxWidth: number): void {
  ...
  if (
    (node.getStyle().maxDimensions[YGDimension.Width].value != value.value && value.unit != YGUnit.Undefined) ||
...
export function YGNodeStyleSetMaxHeight(node: YGNode, maxHeight: number): void {
  ...
  if (
    (node.getStyle().maxDimensions[YGDimension.Height].value != value.value && value.unit != YGUnit.Undefined) ||
...
export function YGIsBaselineLayout(node: YGNode): boolean {
  ...
  for (let i = 0; i < childCount; i++) {
    const child: YGNode = YGNodeGetChild(node, i);
    if (child.getStyle().positionType == YGPositionType.Relative && child.getStyle().alignSelf == YGAlign.Baseline) {
      return true;
    }
  }
...
export function YGNodeComputeFlexBasisForChild(
  ...
  if (!resolvedFlexBasis.isUndefined() && !YGFloatIsUndefined(mainAxisSize)) {
    ...
    child.setLayoutComputedFlexBasis(
      YGFloatOptionalMax(YGResolveValue(child.getResolvedDimension(YGDimension.Width), ownerWidth), paddingAndBorder),
    );
    ...
    child.setLayoutComputedFlexBasis(
      YGFloatOptionalMax(YGResolveValue(child.getResolvedDimension(YGDimension.Height), ownerHeight), paddingAndBorder),
    );
  } else {
...
export function YGNodeAbsoluteLayoutChild(
  ...
  if (child.isTrailingPosDefined(mainAxis) && !child.isLeadingPositionDefined(mainAxis)) {
    ...
    child.setLayoutPosition(
      (node.getLayout().measuredDimensions[dim[mainAxis]] - child.getLayout().measuredDimensions[dim[mainAxis]]) / 2.0,
      leading[mainAxis],
    );
    ...
    switch (justifyContent) {
      ...
      case YGJustify.SpaceBetween:
        if (collectedFlexItemsValues.itemsOnLine > 1) {
          betweenMainDim =
            YGFloatMax(collectedFlexItemsValues.remainingFreeSpace, 0) / (collectedFlexItemsValues.itemsOnLine - 1);
        } else {
          betweenMainDim = 0;
        }
        break;
...
export function YGNodelayoutImpl(
              ...
              case YGAlign.Baseline: {
                child.setLayoutPosition(
                  currentLead +
                    ...
                    YGBaseline(child) +
                    YGUnwrapFloatOptional(child.getLeadingPosition(YGFlexDirection.Column, availableInnerCrossDim)),
                  YGEdge.Top,
                );
    ...
    node.setLayoutMeasuredDimension(
      YGFloatMax(
        YGFloatMin(
          ...
          YGUnwrapFloatOptional(YGNodeBoundAxisWithinMinAndMax(node, mainAxis, maxLineMainDim, mainAxisownerSize)),
        ),
  ...
  if (
    ...
    node.setLayoutMeasuredDimension(
      YGNodeBoundAxis(node, crossAxis, totalLineCrossDim + paddingAndBorderAxisCross, crossAxisownerSize, ownerWidth),
      dim[crossAxis],
    );
